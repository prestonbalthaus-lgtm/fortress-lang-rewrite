...
All [legacy]: fortressc has no set type at all and rejects both enclosers, "expected an expression, found LeftBar" and "expected an expression, found LBrace".  ⚠ 2026-08-23: still refused, but the messages moved -- both enclosers PARSE now (`opr` declarations landed) and come back as `` unknown name `<|_|>` `` and `` unknown name `{_}` ``, which names the missing declaration rather than the bracket.  ⚠ 2026-08-23 (later): AND A COMPREHENSION PARSES in every bracket. `<| e | x <- g, p |>` and `{ e | x <- g }` are ONE production in 1.0 (`DelimitedExpr.rats:290-314`), so the bracket pair is carried as the operator name and nothing is list-specific. Static arguments go INSIDE the opener -- `<|[\E\] e | ... |>`. A guard is a generator clause with NO binder, which is 1.0's own representation. The separator is a bare `|` with WHITESPACE ON BOTH SIDES (`wr bar wr`); `<|x|x<-s|>` does not parse in 1.0 either. The LOWERING is refused by name: a comprehension accumulates an unknown number of elements and nothing in this backend grows storage.  ⚠ 2026-08-23 (later still): THE LIST FORM IS BUILT. `<|[\T\] e | x <- lo:hi, p |>` lowers onto a real MONOMORPHIZED `List[\T\]`, minted into the component from `crates/types/src/List.fss` and stamped by expansion like any other instantiation. Storage is an ordinary `Array[\T\]` and growth is `array(2 n + 8)` plus a copy, so it is still the single allocation path. `size()`, `get(i)` and `append(x)` are its methods. The element type is WRITTEN, never inferred: on the comprehension, or on the binding, field or return type it fills. The lowering is SEQUENTIAL -- a `while`, not a `for` -- because a `for` body is outlined and may run on several workers, and a list comprehension's order is its generator's. Ranges `lo:hi` and `lo#n`, guards and several generators all work, AND THE BINDER IS ZZ64 like a `for` bound, so `<|[\ZZ32\] x | x <- 1:10 |>` does not check. A `{_}` set comprehension, a map one and a generator over a COLLECTION are still refused, each by its own name.  ⚠ **2026-08-25: THE SET FORM IS BUILT, AND SO IS THE SET LITERAL.** `{ e | x <- lo:hi, p }` lowers onto a minted `Set[\T\]` (`crates/types/src/Set.fss`) exactly the way the list form lowers onto `List[\T\]`, and `{a, b, c}` -- which parses as a CALL to an enclosing operator named `{_}`, not as a literal node -- lowers onto the same collection. Its methods are `size()`, `opr [i]`, `contains(x)`, `insert(x)` and `opr IN(x, self)`. DUPLICATES COLLAPSE and the order kept is INSERTION ORDER OF FIRST OCCURRENCE: 1.0's `Library/Set.fsi:56` bounds the element by `StandardTotalOrder[\T\]` and keeps it SORTED, and there is no first-class ordering to demand of a written static argument here, so that is a NAMED DEVIATION. Membership is a linear scan over `=`, so `Set[\ZZ32\]` and `Set[\String\]` work and an object element needs its own `=`. THE ELEMENT TYPE IS STILL WRITTEN, NEVER INFERRED -- on the brackets (`{[\T\] a, b}`) or on the slot -- and for the literal that is STRUCTURAL rather than stylistic: `Set[\T\]` is STAMPED by monomorphization, which runs before the type checker exists, so an element type discoverable only by typing the elements cannot be stamped at all. A generator over a COLLECTION works. A MAP comprehension, `{[\K,V\] ...}` with two static arguments, is refused by name on that arity; an ARRAY comprehension never reaches the comprehension parse at all -- `[ x | x <- 1:3 ]` stops at `expected `]`, found Bar`.  ⚠ **2026-08-25 (later): THE MAP FORMS ARE BUILT TOO.** `{k |-> v, ...}` and `{k |-> v | x <- g}` lower onto a minted `Map[\K,V\]` (`crates/types/src/Map.fss`), two parallel arrays rather than an array of pairs because a pair would be a tuple VALUE and this backend flattens tuples instead of boxing them. `size()`, `opr [k]`, `contains(k)` and `insert(k, v)` are its methods, and a later `insert` at a present key REPLACES its value. WHICH COLLECTION `{ }` BUILDS IS DECIDED BY THE ELEMENT, not the brackets: `{a, b}` is a set and `{k |-> v}` is a map, which is how 1.0 spells them too. Mixing the two in one literal is refused, as is a map's two static arguments over a body that is not a mapping, and a mapping body in the `<| |>` brackets names the BODY. `|->` IS NOT A LEXER TOKEN: it is `Bar` glued to `Minus` glued to `Gt`, re-glued by the parser on span adjacency exactly as `->` and `<-` are. Measured before it was written -- 439 corpus sites write `|->` and ZERO write `| ->`. A mapping written anywhere a VALUE belongs is refused by name: it is one entry of a map and has no representation of its own.  ⚠ **2026-08-25 (later still): AND THE ARRAY FORM.** `a: T[n] = [ i |-> e | i <- lo:hi ]` FILLS the array the binding declares -- it is not a minted collection, so there is no builder and the fill is an ordinary subscript store. THE EXTENT COMES OFF THE SLOT and the body must be `index |-> value`: an array is sized once, at construction, a guard makes the count unknowable until the walk is over, and there is no `append` to accumulate with. `T[2,3]`, `T^3` and `T[0#n]` are refused with the extent message. It is reached from the AGGREGATE parser and not from the enclosing-operator one, because `[` opens an aggregate first -- which is why `[ x | x <- 1:3 ]` used to stop at `expected `]`, found Bar`.
