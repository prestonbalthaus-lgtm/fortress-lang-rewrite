...
if opt.get('refresh_lists'):
    with open(REFUSE_LIST, 'w') as f:
        f.write('# Corpus files the legacy implementation REFUSED and this\n'
                '# compiler ACCEPTS. Generated by tools/oracle-gate.sh\n'
                '# --refresh-lists; regenerate only to REMOVE a line, never to\n'
                '# add one -- a new acceptance is a regression, not a baseline.\n')
        f.write('\n'.join(accepted) + '\n')
    with open(SIGNAL_LIST, 'w') as f:
        f.write('# Corpus binaries that do not exit 0 or 1. Generated by\n'
