#!/usr/bin/env bash
#
# The Char gate: character literals and the `Char` type.
#
# Five things cargo cannot check on its own: that every shape the specification
...
# that the legacy's own recorded output for `Char.fss` is reproduced.
#
...
#
#   ./tools/char-gate.sh              run the gate
#   ./tools/char-gate.sh --selftest   only prove the assertions can refuse
#   ./tools/char-gate.sh --mutate     break the compiler four ways and prove
...
refusals() {
    ...
    for entry in \
        ...
        "badforbiddenchar|a character literal holds one character|a raw control character is refused by name"; do
