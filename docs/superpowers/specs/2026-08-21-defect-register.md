...
| DEV-13 | **The shipped 1.0 library violates the `comprises` rule the legacy enforces.** `Library/FortressLibrary.fsi` declares `trait QQ ... comprises { ... }` and then `trait AnyIntegral extends { QQ }` — byte for byte the shape the legacy refuses in `Compiled5.x.fsi`. SIX declarations across THREE api files (Library/FortressLibrary.fsi, CompilerLibrary/FortressLibrary.fsi, Library/Reflect.fsi). No `.test` checks the library, so the legacy never ran its own static rules over its own source. The rule is KEPT: the root PARSES in full and does not CHECK, and `resolve()` needs an api to parse, not to check. There is no syntactic separation between the library's shape and the must-fail files' — I looked | `traits.tex:236-241`, measured | Preston deciding the library's defect outweighs two must-fail refusals — the QuickSort/DEV-6 template |
